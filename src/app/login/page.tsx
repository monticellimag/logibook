...
const DEV_CREDENTIALS: Record<LoginMode, { label: string; email: string; password: string; badge?: string }[]> = {
  ...
  admin: [
    ...
    { label: "Monticelli", email: "monticelli@azienda.it", password: "password", badge: "Hub" },
    { label: "S. Giorgio", email: "sangiorgiobi@azienda.it", password: "password", badge: "Hub" },
    { label: "Oppeano 1", email: "oppeano1@azienda.it", password: "password", badge: "Hub" },
  ],
  gate: [
    { label: "Super Admin", email: "admin@logibook.local", password: "admin", badge: "Gate" },
    { label: "Gate Monticelli", email: "gate.monticelli@azienda.it", password: "password", badge: "Gate" },
    { label: "Gate S.Giorgio", email: "gate.sangiorgiobi@azienda.it", password: "password", badge: "Gate" },
  ],
};
...
function LoginContent() {
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5 animate-fade-in-up">
              <div>
                ...
                <input type="text" required autoComplete="organization"
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl focus:ring-2 focus:ring-brand-action outline-none text-white transition-all placeholder:text-slate-600 font-bold"
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl focus:ring-2 focus:ring-brand-action outline-none text-white transition-all placeholder:text-slate-600 resize-none h-24 font-bold"
            ...
            <>
              <div>
                ...
                <input type="email" required
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl focus:ring-2 focus:ring-brand-action outline-none text-white transition-all placeholder:text-slate-600"
                  value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})}
                  placeholder={mode === 'vettore' ? 'tuamail@azienda.it' : 'admin@azienda.it'}
                />
              </div>
          ...
          <button
            ...
            className={`w-full py-5 rounded-2xl shadow-2xl text-lg font-black text-white transition-all active:scale-[0.98]
              ${mode === 'admin' ? 'bg-slate-800 hover:bg-slate-700' : mode === 'gate' ? 'bg-brand-action hover:opacity-90' : 'bg-indigo-600 hover:bg-indigo-500'}
      ...
      {mode === 'vettore' && !requestSent && (
        <p className="mt-8 text-center text-slate-400 font-medium">
          ...
          <button 
            ...
            className="font-black text-white hover:underline decoration-brand-action underline-offset-4 decoration-2"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 px-4 max-w-6xl w-full z-10 mb-20 sm:mb-32">
        {[
          ...
          { 
            ...
            color: "bg-brand-action/20",
            iconColor: "text-brand-action",
            hoverBorder: "hover:border-brand-action/50"
          }
        ].map((item) => (
          <button
            key={item.id}
          ...
          >
            ...
            <h2 className="text-2xl font-black text-white mb-3 group-hover:text-brand-action transition-colors">{item.title}</h2>
      ...
      <div ref={featuresRef} className="w-full bg-slate-50/95 backdrop-blur-md rounded-t-[3rem] py-24 px-4 sm:px-8 shadow-[0_-20px_50px_-12px_rgba(0,0,0,0.25)] relative z-10 border-t border-white/20">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20">
            ...
            <div className="w-24 h-1 bg-brand-action mx-auto rounded-full"></div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
            {[
              ...
              <div key={idx} className="flex flex-col items-center text-center group">
                <div className="w-16 h-16 rounded-2xl bg-white shadow-xl flex items-center justify-center mb-6 text-slate-700 group-hover:text-brand-action group-hover:-translate-y-2 group-hover:shadow-brand-action/20 transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto mt-24 pt-8 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-6 text-sm font-medium text-slate-500 pb-8">
          <div className="flex flex-wrap justify-center gap-8">
            <Link href="/cookies-policy" className="hover:text-brand-action transition-colors">Cookies Policy</Link>
            <Link href="/privacy-policy" className="hover:text-brand-action transition-colors">Privacy Policy</Link>
          </div>
          <div className="flex flex-col items-end gap-1">
            <span>&copy; {new Date().getFullYear()} Tutti i diritti riservati.</span>
            <span>Realizzato da <a href="https://www.presenzadigitale.com/" target="_blank" rel="noopener noreferrer" className="hover:text-brand-action font-bold transition-colors">Presenza Digitale</a></span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="relative min-h-screen flex flex-col overflow-x-hidden selection:bg-brand-action selection:text-white">
      {/* Dynamic Background */}
      <div 
